...
class Listing(models.Model):
    ...
    def __str__(self):
        return f"{self.title} - {self.starting_bid}FCFA)"
...
class Bid(models.Model):
    ...
    def __str__(self):
        return f"User #{self.bid_user.id} bid {self.bid_amount}FCFA on auction #{self.bid_auction.id}"
...
class Bid_win(models.Model):
    bid_users = models.ForeignKey(User, on_delete=models.CASCADE, related_name="bid_users")
    bid_auctions = models.ForeignKey(Auction, on_delete=models.CASCADE, related_name="bid_auctions")
    bid_amounts = models.DecimalField(max_digits=8, decimal_places=2)
