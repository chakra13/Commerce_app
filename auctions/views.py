...
def listing(request, listing_path):
    ...
    if request.method == "POST":
        ...
        return render(request, "auctions/bid_list.html",{
            "listing": listing,
            "auction": auction,
            "watching": watching,
            "bidding": bidding,
    ...
    if request.method == "POST":
        ...
        if close_request == "Close":
            ...
            if winner == None:
                ...
                messages.success(request, f"Going... Going... Gone! Sold '{closing.listing_auction.title}' for {winner.bid_amount}FCFA.")
                return redirect("index")
            return redirect("index")
    ...
    if request.method == "POST":
        ...
        if auction.bidding_open:
            ...
            if new_bid < listing.starting_bid or current_bids.filter(bid_amount__gte=new_bid):
                user.watching.add(auction)
                messages.error(request, f"Sorry, your bid of {new_bid}FCFA, wasn\'t quite enough - try again.")
                return redirect("listing", listing_path=listing_path)
            else:
                user = User.objects.get(pk=int(request.user.id))
                ...
                auction.save()
                user.watching.add(auction)
                messages.success(request, f"You're winning! Bid of {new_bid}FCFA placed for '{listing.title}'.")
                return redirect("index")
        else:
